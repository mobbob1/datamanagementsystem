<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white">Edit User</h2>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('users.update', $user) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            @method('PUT')

            <div>
                <x-input-label value="Name" />
                <x-text-input value="{{ $user->name }}" class="w-full" disabled />
            </div>
            <div>
                <x-input-label value="Email" />
                <x-text-input value="{{ $user->email }}" class="w-full" disabled />
            </div>
            ...
            <div>
                ...
                <select name="role_id" class="w-full border-orc-navy/30 focus:border-orc-teal focus:ring-orc-teal rounded-md" required>
                    @foreach($roles as $r)
                        <option value="{{ $r->id }}" @selected(old('role_id', optional($user->role)->id) == $r->id)>{{ $r->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="organization_unit_id" class="w-full border-orc-navy/30 focus:border-orc-teal focus:ring-orc-teal rounded-md">
                    ...
                    @foreach($units as $u)
                        <option value="{{ $u->id }}" @selected(old('organization_unit_id', optional($user->organizationUnit)->id) == $u->id)>{{ $u->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <x-text-input type="number" name="clearance_level" min="1" max="10" value="{{ old('clearance_level', $user->clearance_level) }}" class="w-full" required />
            </div>
            <div class="md:col-span-2">
                <x-input-label value="Phone" />
                <x-text-input name="phone" value="{{ old('phone', $user->phone) }}" class="w-full" />
            </div>
