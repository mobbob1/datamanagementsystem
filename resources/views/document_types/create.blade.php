<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white">Create Document Type</h2>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('document-types.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf

            <div>
                <x-input-label value="Key" />
                <x-text-input name="key" value="{{ old('key') }}" class="w-full" required />
            </div>
            <div>
            ...
            <div>
                ...
                <select name="default_classification_id" class="w-full border-orc-navy/30 focus:border-orc-teal focus:ring-orc-teal rounded-md">
                    ...
                    @foreach($classifications as $c)
                        <option value="{{ $c->id }}" @selected(old('default_classification_id') == $c->id)>{{ $c->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <x-input-label value="Default Retention (months)" />
                <x-text-input type="number" name="default_retention_months" min="0" value="{{ old('default_retention_months') }}" class="w-full" />
            </div>
            <div class="md:col-span-2">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_active" value="1" class="rounded border-orc-navy/30 text-orc-teal shadow-sm focus:ring-orc-teal" {{ old('is_active', true) ? 'checked' : '' }}>
