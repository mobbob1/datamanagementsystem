<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl">Edit Organization Unit</h2>
            <a href="{{ route('organization-units.index') }}" class="text-sm text-orc-navy hover:underline">Back</a>
        </div>
    </x-slot>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-3xl">
        <form method="POST" action="{{ route('organization-units.update', $unit) }}" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select id="type" name="type" class="mt-1 block w-full border-orc-navy/30 focus:border-orc-teal focus:ring-orc-teal rounded-md">
                        ...
                        @foreach ($types as $k => $v)
                            <option value="{{ $k }}" {{ old('type', $unit->type) === $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="parent_id" name="parent_id" class="mt-1 block w-full border-orc-navy/30 focus:border-orc-teal focus:ring-orc-teal rounded-md">
                        ...
                        @foreach ($parents as $p)
                            <option value="{{ $p->id }}" {{ (int) old('parent_id', $unit->parent_id) === (int) $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-end">
                    <label class="inline-flex items-center mt-6">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-orc-navy/30 text-orc-teal shadow-sm focus:ring-orc-teal" {{ old('is_active', $unit->is_active) ? 'checked' : '' }}>
