<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl">Add Classification</h2>
            <a href="{{ route('classifications.index') }}" class="text-sm text-orc-navy hover:underline">Back</a>
        </div>
    </x-slot>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-3xl">
        <form method="POST" action="{{ route('classifications.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-orc-navy/30 focus:border-orc-teal focus:ring-orc-teal rounded-md">{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="clearance_level" name="clearance_level" type="number" min="1" max="10" class="mt-1 block w-full" value="{{ old('clearance_level', 1) }}" required />
            ...
            <div class="flex items-center justify-end space-x-3">
                ...
                <x-primary-button>Save Classification</x-primary-button>
            </div>
        </form>
    </div>
</x-app-layout>
